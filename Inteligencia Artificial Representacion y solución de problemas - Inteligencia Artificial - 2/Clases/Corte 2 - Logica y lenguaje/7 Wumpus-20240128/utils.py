...
class Plot :
    ...
    def plot_histogram_rewards(self, file:str=None) -> plt.axis:
        ...
        ax.grid()
        columnas = ['model', 'episode']
        if 'simulation' in self.data.columns:
            columnas.append('simulation')
        if 'environment' in self.data.columns:
            columnas.append('environment')
        if vs_models:
            df = self.data.groupby(columnas).agg({'reward':'sum', 'done':'sum'}).reset_index()
...
class Experiment :
    ...
    def run_experiment(self, \
        ...
        for environment in tqdm(self.test_suite, desc='Testing agents over environment'):
            ...
            for k in tqdm(range(self.num_simulations), desc='\t Running simulations', leave=False):
                if verbose > 0:
